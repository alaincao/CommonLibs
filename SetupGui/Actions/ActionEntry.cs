...
namespace CommonLibs.WPF.SetupGui.Actions
{
	public class ActionEntry
	{
		...
		public bool								Success					{ get { return (Status==Statuses.Finished) && (Errors.Count==0) && (Exceptions.Count==0); } }  // NB: No need to lock{} since if action has Finished then only the main thread will access this entry
		public bool								HasWarnings				{ get { lock(WarningsLocker){ return Warnings.Count > 0; } } }
		public bool								HasErrors				{ get { lock(ErrorsLocker){ return Errors.Count > 0; } } }
		public bool								HasExceptions			{ get { lock(ExceptionsLocker){ return Exceptions.Count > 0; } } }

		private StringBuilder					Logs					= new StringBuilder();
		private object							LogsLocker				= new object();
		private List<string>					Errors					= new List<string>();
		private object							ErrorsLocker			= new object();
		private List<string>					Warnings				= new List<string>();
		private object							WarningsLocker			= new object();
		private List<Exception>					Exceptions				= new List<Exception>();
		private object							ExceptionsLocker		= new object();
...
HERE: Définir un SynchronizingObject dans le ActionsManager
		Interdire les Background et les External si il n'y a pas de SynchronizingObject disponible
		Utiliser les SynchronizingObject à la place des Logs/ExceptionLoger et autres
					System.Diagnostics.Debug.Assert( (ExternalHelper != null) && (ExternalHelper.CallerProcess != null) && (ExternalHelper.CallerProcess.SynchronizingObject != null) );
					if( ExternalHelper.CallerProcess.SynchronizingObject.InvokeRequired )
						ExternalHelper.CallerProcess.SynchronizingObject.Invoke( new Action( ()=>LogLine(line) ), null );
					else
						goto case ActionTypes.Regular;
					break;
